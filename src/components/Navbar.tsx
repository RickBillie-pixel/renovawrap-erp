...
export const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const scrollY = Math.max(mainElement?.scrollTop || 0, window.scrollY);
    ...
    <>
      ...
      >
        <div className="container-wide flex items-center justify-between">
          ...
          >
            {isMobileMenuOpen ? (
              <X className={`w-6 h-6 transition-colors duration-300 ${isHomePage && !isScrolled ? "text-white" : "text-foreground"
                }`} />
            ) : (
              <Menu className={`w-6 h-6 transition-colors duration-300 ${isHomePage && !isScrolled ? "text-white" : "text-foreground"
                }`} />
            )}
          </button>
        </div>
          ...
          <div className="flex flex-col gap-1 sm:gap-2">
            {navLinks.map((link, index) => (
              ...
              >
                <Link
                  to={link.path}
                  className={`block font-display text-2xl sm:text-3xl font-semibold py-3 sm:py-4 transition-colors ${location.pathname === link.path
                      ? "text-primary"
                      : "text-foreground"
                    }`}
                >
